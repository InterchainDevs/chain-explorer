<template> 
    <v-row no-gutters>
      <v-col 
        ...
        sm="4"
      >
        <v-sheet border rounded="lg" class="mr-4 mb-4 pa-2">
          Block Height
      ...
      <v-sheet border rounded="lg" class="mr-4 mb-4 pa-2">
          ...
          <div class="text-end">
            <v-chip 
              label
            >
            {{ blockInfo.sdk_block?.last_commit.signatures.length }} / {{ totalValidators }}
            </v-chip>                  
          </div>
      </v-sheet>
      </v-col>
...
export default {
  ...
  async mounted() {
    ...
    let getBlockInfo = await axios.get('https://lcd.bitcanna.io/cosmos/base/tendermint/v1beta1/blocks/'+ this.blockId) 
    this.blockInfo = getBlockInfo.data;

    for (let i = 0; i < this.blockInfo.block.data.txs.length; i++) {
      const decodedTx = decodeTxRaw(fromBase64(this.blockInfo.block.data.txs[i]));
      ...
      for (let j = 0; j < decodedTx.body.messages.length; j++) {
        ...
        if (foundMsgType) {
          ...
          console.log(msg);
          this.allTransactions.push({type: decodedTx.body.messages[j].typeUrl, message: msg});
        } else {
          console.log("Unknown message type: " + decodedTx.body.messages[j].typeUrl);
        }
      } 
    }
