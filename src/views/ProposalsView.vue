<template> 
    ...
    <v-row no-gutters>
      <v-col 
        ...
        sm="12"
      >
        <v-sheet border rounded="lg" class=" pa-2 animate__animated animate__backInRight">
          <v-table>
            <thead>
              <tr>
                <th class="text-left">
            ...
            <tbody>
              <tr
                v-for="item in getProposals"
                :key="item.name"
              >
                <td>#{{ item.proposal_id }}</td>
                <td>{{ item.content.title }}</td>
                <td>{{ moment(item.submit_time).format('MMMM Do YYYY, h:mm:ss a') }}</td>
...
export default {
  ...
  async mounted() {
    ...
    let getPrpoposals = await axios.get("https://lcd.bitcanna.io/cosmos/gov/v1beta1/proposals?proposal_status=PROPOSAL_STATUS_UNSPECIFIED") 
