  ...
  actions: {
    ...
    async getSdkVersion() {
      ...
      async getBankModule(addrWallet) {
        ...
        let spendableBalances = await queryBank.SpendableBalances({ address: addrWallet });
        let allBalances = await queryBank.AllBalances({ address: addrWallet });
  
        const found = spendableBalances.balances.find(element => element.denom === cosmosConfig[2].coinLookup.chainDenom);
        // TODO: fix this
        let returnValue = ''
        if ( found?.amount > 0){
        ...
        let totalSupply = await queryBank.SupplyOf({ denom: cosmosConfig[2].coinLookup.chainDenom })
      ...
      async getStakingModule(addrWallet) {
        ...
        let delegatorValidators = await queryStaking.DelegatorDelegations({ delegatorAddr: addrWallet, pagination: {
          countTotal: false,
          key: '',
        ...
        let total = 0;
        let allUnbound = await queryStaking.DelegatorUnbondingDelegations({ delegatorAddr: addrWallet });
        let totalUnbound = 0;
      ...
      async getDistribModule(addrWallet) {
        ...
        const queryDistribResult = await queryDistrib.DelegationTotalRewards({ delegatorAddress: addrWallet });
        const found = queryDistribResult.total.find(element => element.denom === cosmosConfig[2].coinLookup.chainDenom);
        let returnValue = ''
        ...
        const queryWithdrawAddressResult = await queryDistrib.DelegatorWithdrawAddress({ delegatorAddress: addrWallet });
      ...
      async getAddressTx(addrWallet, page) {
        const getRecipient = await axios(cosmosConfig[2].apiURL + '/cosmos/tx/v1beta1/txs?events=transfer.recipient=%27'+addrWallet+'%27&page='+page+'&limit=10&order_by=2')
      ...
      async getAllValidators() {
        let getAllValidators = await fetch(cosmosConfig[2].apiURL +'/cosmos/staking/v1beta1/validators?pagination.limit=500&status=BOND_STATUS_BONDED')
        const getInflation = await axios(cosmosConfig[2].apiURL + '/cosmos/mint/v1beta1/inflation')
        const allsupply = await axios(cosmosConfig[2].apiURL + `/cosmos/bank/v1beta1/supply/by_denom?denom=ubcna`)
        const totalBounded = await axios(cosmosConfig[2].apiURL + `/cosmos/staking/v1beta1/pool`)
        ...
        for (let item of result.reverse()) {        
          try {         
            ...
            item.valApr = ((this.allsupply / 1000000 * this.inflation) * item.votingPowerPc / 100 * (100 - (item.commission.commissionRates.rate / 10000000000000000))) / (item.tokens / 1000000)
          } catch (error) {
            console.error(error);
          }
        }
      ...
      async getParams() {
        ...
        let finalMint = [{
          ...
          annualProvisions: getAnnualProvisions.data.annual_provisions / 1000000,
      ...
      async getDetailsValidator(address) {
        const querystaking = new staking.QueryClientImpl(this.rpcClient);
        let detailValidator = await querystaking.Validator({ validatorAddr: address });
        this.detailValidator = detailValidator.validator;

        let allValidatorDelegations = await querystaking.ValidatorDelegations({ validatorAddr: address, pagination: {
          countTotal: false,
          key: '',
          offset: Long.fromNumber(0, true),
          limit: Long.fromNumber(200, true),
        ...
        this.allValidatorDelegations = allValidatorDelegations.delegationResponses;
      },
