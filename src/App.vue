    ...
    <v-navigation-drawer :width="352" floating>
      <v-table>
          <tbody>
            <tr>
              <td>SDK Version</td>
              <td class="text-right"><v-chip label>{{ store.sdkVersion }}</v-chip></td>
            </tr>
            <tr>
              <td>IBC Version</td>
              <td class="text-right">
      ...
      <div >
        ...
        >
          ...
          <template v-slot:subtitle>
            <div v-html="moment(n.timestamp).format('MMMM Do YYYY, h:mm:ss a')"></div>
          </template>
        </v-list-item>
        <v-divider ></v-divider>
        </div>
  ...
  watch: {
    searchData(hash) {
      try {
        ...
        this.searchData = '';
        this.$router.push({ path: '/address/', name: "detailaddress", params: { address: encode } });
        return;
      } catch (error) {
        //console.log(error);
      } 
